...
export function registerRoomHandlers(io: Server, socket: Socket) {
  ...
  socket.on("joinRoom", (data: any, cb: any) => {
    ...
    try {
      ...
      if (!room.hasPlayer(playerId)) {
        ...
        console.log(`[JOIN ROOM] Adding new player '${player.name}' (${player.id}) to room ${roomCode}`);
  ...
  socket.on("setPlayerSeated", (data: any, cb: any) => {
    ...
    if (seatedPlayers.length >= 2) {
      if (!room.isGameTimerRunning()) {
        ...
        console.log(`[SET SEATED] Started game timer for roomCode=${roomCode}, deadline=${deadline}`);
      }
    } else {
  ...
  socket.on("submitWord", (data: any, cb: any) => {
    ...
    if (!currentPlayer) return callback({
      success: false,
      error: "No current player."
    });
    if (currentPlayer.id !== playerId) return callback({
      success: false,
      error: "Not your turn."
    });
