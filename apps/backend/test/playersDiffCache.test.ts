...
import { computePlayersDiff, resetPlayersDiffCache } from '../src/game/orchestration/playersDiffCache';

const rules: GameRoomRules = { maxLives:3, bonusTemplate:new Array(26).fill(1), minTurnDuration:5, minWordsPerPrompt:1 };
...
describe('playersDiffCache', () => {
  beforeEach(() => { resetPlayersDiffCache(); });
  ...
  it('detects updates and removals', () => {
    ...
    if (diff) {
      expect(diff.updated.find(u => u.id === 'A')?.changes.isSeated).toBe(true);
