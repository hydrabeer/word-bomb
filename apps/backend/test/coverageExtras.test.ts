...
import { loadDictionary, isValidWord, getRandomFragment } from '../src/dictionary';
...
function requireId(id: string | undefined): string { if (!id) throw new Error('id'); return id; }
...
describe('coverage extras', () => {
  ...
  const { game } = makeRoomWithGame();
      ...
      const engine = new GameEngine({
        game,
        emit: ((e: unknown, payload: unknown) => { emits.push({ event: e as string, payload }); }) as EmitTest,
        onTurnStarted: () => void 0,
        onGameEnded: () => void 0,
      });
  const current = game.getCurrentPlayer();
  const notTurn = game.players[1];
      expect(engine.submitWord(notTurn.id, 'aaaa')).toEqual({ success: false, error: 'Not your turn.' });
      expect(engine.submitWord(current.id, 'a')).toEqual({ success: false, error: 'Invalid word (too short).' });
      expect(engine.submitWord(current.id, 'zz')).toEqual({ success: false, error: "Word doesn't contain the fragment." });
    });
    ...
    it('advances turn and emits acceptance on valid word (mock validity)', () => {
      ...
      vi.mock('../src/dictionary', async () => {
        const mod = await vi.importActual<typeof import('../src/dictionary')>('../src/dictionary');
        return { ...mod, isValidWord: () => true, getRandomFragment: () => 'aa' };
      });
  ...
  const { game } = makeRoomWithGame();
      ...
      const engine = new GameEngine({
        game,
        emit: ((e: unknown) => { emits.push(e as string); }) as EmitTest,
        onTurnStarted: () => void 0,
        onGameEnded: () => void 0,
      });
      const current = game.getCurrentPlayer();
  ...
  await new Promise<void>((r) => { s.once('connect', () => { r(); }); });
      ...
      s.emit('joinRoom', { roomCode: 'NOPE', playerId: requireId(s.id), name: 'A' }, (r: { success: boolean; error?: string }) => { response = r; });
      await new Promise(r => setTimeout(r, 20));
  expect(response?.success).toBe(false);
      // create room then invalid name (>20 chars)
      roomManager.create('ZZZZ', { maxLives: 3, bonusTemplate: new Array(26).fill(1), minTurnDuration: 1, minWordsPerPrompt: 1 });
      s.emit('joinRoom', { roomCode: 'ZZZZ', playerId: requireId(s.id), name: 'x'.repeat(25) }, (r: { success: boolean; error?: string }) => { response = r; });
      await new Promise(r => setTimeout(r, 20));
  expect(response?.success).toBe(false);
    });
    ...
    it('removes inactive player after disconnect grace timeout (short grace)', async () => {
      ...
      const ctx = useServer();
      roomManager.create('TIME', { maxLives: 3, bonusTemplate: new Array(26).fill(1), minTurnDuration: 1, minWordsPerPrompt: 1 });
      const observer = ctx.createClient();
      const pSock = ctx.createClient();
      await Promise.all([
        new Promise<void>((r) => { observer.once('connect', () => { r(); }); }),
        new Promise<void>((r) => { pSock.once('connect', () => { r(); }); }),
      ]);
      const pSockId = requireId(pSock.id);
  const diffs: { removed: string[] }[] = [];
      observer.emit('joinRoom', { roomCode: 'TIME', playerId: requireId(observer.id), name: 'Obs' });
      observer.on('playersDiff', d => diffs.push(d));
  pSock.emit('joinRoom', { roomCode: 'TIME', playerId: pSockId, name: 'GoneSoon' });
    ...
    it('switches rooms properly (leave previous)', async () => {
      const ctx = useServer();
      roomManager.create('AAAA', { maxLives: 3, bonusTemplate: new Array(26).fill(1), minTurnDuration: 1, minWordsPerPrompt: 1 });
      roomManager.create('BBBB', { maxLives: 3, bonusTemplate: new Array(26).fill(1), minTurnDuration: 1, minWordsPerPrompt: 1 });
      const s = ctx.createClient();
  await new Promise<void>((r) => { s.once('connect', () => { r(); }); });
      s.emit('joinRoom', { roomCode: 'AAAA', playerId: requireId(s.id), name: 'Mover' });
      await new Promise(r => setTimeout(r, 20));
      s.emit('joinRoom', { roomCode: 'BBBB', playerId: requireId(s.id), name: 'Mover' });
      await new Promise(r => setTimeout(r, 20));
